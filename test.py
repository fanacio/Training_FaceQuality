...
parser.add_argument('--backbone', default='pre_weights/backbone.pth', type=str, metavar='PATH',
                    help='path to backbone model')
parser.add_argument('--quality', default='pre_weights/quality.pth', type=str, metavar='PATH',
                    help='path to quality model')
parser.add_argument('--file', default='test_faces', type=str, metavar=' PATH',
...
def get_face_quality(backbone, quality, device, img, pytorch2onnx):
    ...
    with torch.no_grad():
        ...
        # pytorch to onnx
        if pytorch2onnx:
            torch.onnx.export(backbone,ccropped.to(device),"./backbone.onnx")
            torch.onnx.export(quality,fc,"./quality.onnx")
...
def main(args):
    ...
    if os.path.isfile(args.file):
        ...
        if image is None or image.shape[0] == 0:
            ...
            return
        quality = get_face_quality(BACKBONE, QUALITY, DEVICE, image, args.pytorch2onnx)
